...
public class Checker {
	...
	public Checker copy(Checker other, int x_top, int y_top, int x_to, int y_to) {
		...
		// 		int name = other.getBoard()[i][j].getName();
		// 		this.board[i][j] = new Coordinate(i,j,name);
		...
		// 			// int name = bl.getName();
		// 			Coordinate top = new Coordinate(x, y,name);
		// 			Coordinate bottom = new Coordinate(xi, yi,name);
		// 			// Block bl2 = new Block(bl.getTop(), bl.getBottom(), bl.getName());
		// 			Block bl2 = new Block(top, bottom, name);
...
*/
	...
	/*	public void move(Coordinate toMoveTo , boolean check){
		...
		public void move(Coordinate toMoveTo, boolean flag){
		...
		public boolean canMakeMove(Coordinate toMoveTo)  {  // 
			...
			if(goals.contains(board[toMoveTo.x + x_length -1][toMoveTo.y + y_length - 1])) {
				...
				System.out.println("HHHHHHHHHHHHHHHHHHHHHHHHHhhh + "+ temp);
				System.out.println("this = " + toString());
				System.out.println("toMoveTo = "+ toMoveTo);
			}
		...
		public boolean check() {
			if(blocks.get(board[1][10]) != null) {
				Block bl = blocks.get(board[1][10]);
				String s = bl.toString();
				// if(s.equals("2: 1 8, 2: 1 8")) {
				System.out.println(blocks);
					return true;
				// }
			}
		...
		public void right(Coordinate toMoveTo) {
			...
			for(int i = this.bottom_right.y + 1; i <= toMoveTo.y  && i < board[0].length ; i++) {
				for(int j = this.bottom_right.x; j >= this.top_left.x; j--) {
					System.out.println(board[j][i]);
					// board[j][i] = new Coordinate(i,j,Integer.parseInt(this.name));
